...
export function AuthProvider({ children }) {
  ...
  const fetchProfile = useCallback(async (userId) => {
    ...
    if (!error) {
      ...
      // Don't fail silently — if this errors (e.g. RLS misconfigured, or the
      // signup trigger hasn't created the row yet), leaving profile as null
      // silently makes "Dashboard Admin" mysteriously never show up with no
      // clue why. Logging it means the browser console at least tells you.
      console.error('[AuthContext] Gagal memuat profil user:', error.message)
    }
  }, [])
