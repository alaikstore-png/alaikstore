...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    supabase
      .from('games')
      .select('slug, name, publisher, is_hot')
      .eq('is_active', true)
      .then(({ data, error }) => {
        catalogCache = !error && data && data.length > 0
          ? data.map((g) => ({ slug: g.slug, name: g.name, publisher: g.publisher, hot: g.is_hot }))
                    ...
                    {results.map((r, i) => (
                      ...
                      >
                        <div className="w-9 h-9 rounded-lg overflow-hidden shrink-0">
                          <GameArt slug={r.slug} name={r.name} bannerMode />
                        </div>
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            <div className="relative group">
              <button className="flex items-center gap-2 glass px-3 py-2 rounded-xl text-sm hover:border-neon/40 transition-colors">
                <User className="w-4 h-4 text-neon-light" />
                {profile?.full_name || 'Akun'}
              </button>
              <div className="absolute right-0 mt-2 w-48 glass-card p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all">
                <Link to="/dashboard" className="block px-3 py-2 rounded-lg text-sm hover:bg-white/5">Dashboard</Link>
                <Link to="/riwayat-transaksi" className="block px-3 py-2 rounded-lg text-sm hover:bg-white/5">Riwayat Transaksi</Link>
                {profile?.role === 'admin' && (
                  <Link to="/admin" className="block px-3 py-2 rounded-lg text-sm hover:bg-white/5">Dashboard Admin</Link>
                )}
      ...
      {open && (
        ...
        >
          ...
          <div className="flex gap-3 pt-2">
            {user ? (
              <>
                <Link to="/dashboard" className="btn-outline flex-1 text-center text-sm">Dashboard</Link>
                <button onClick={signOut} className="btn-neon flex-1 text-sm">Keluar</button>
              </>
            ) : (
              <>
